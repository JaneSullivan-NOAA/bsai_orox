...
#' @export
#' 
read_fit <- function(tplfile)
{
  ret <- list()
  parfile <- as.numeric(scan(paste0(tplfile, ".par"), what = '', n=16, quiet=TRUE)[c(6,11,16)])
  ret$nopar <- as.integer(parfile[1])
  ret$nlogl <- parfile[2]
  ret$maxgrad <- parfile[3]
  file <- paste0(tplfile, ".cor")
  if (file.exists(file)) {
    lin <- readLines(file)
...
read_ctl <- function(fn)
{
  ...
  {
    ir <- match(vnam[i],repfile)
    if(i!=nv) irr=match(vnam[i+1],tplfile) else irr=length(tplfile)+1 #next row
    dum=NA
